...
const recipeContent = template
  .replace(/title:\s*(["']?)Nazwa przepisu\1/, `title: "${title}"`)
  .replace(/slug:\s*recipe-name/, `slug: ${slug}`)
  .replace(/image:\s*recipe-name\.jpg/, `image: ${slug}.jpg`)
  .replace(
    /publishedAt:\s*(["']?)YYYY-MM-DDTHH:mm:ss\+02:00\1/,
...
console.log(
  `1. Dodaj zdjęcie do public/images/recipes/ jako ${slug}.png, ${slug}.jpg lub ${slug}.heic`,
);
